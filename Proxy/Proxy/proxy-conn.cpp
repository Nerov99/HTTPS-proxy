 ...
 * @param io_service 
 */
connection::connection(boost::asio::io_service& io_service) : io_service_(io_service),
													 bsocket_(io_service),
													 ssocket_(io_service),
...
//  	std::cout << "start" << std::endl;
	...
	handle_browser_read_headers(boost::system::error_code(), 0);
}
 ...
 * @param len 
 */
void connection::handle_browser_read_headers(const boost::system::error_code& err, size_t len) {
//  	std::cout << "handle_browser_read_headers. Error: " << err << ", len=" << len << std::endl;
	if(!err) {
		...
		if(fHeaders.find("\r\n\r\n") == std::string::npos) { // going to read rest of headers
			async_read(bsocket_, boost::asio::buffer(bbuffer), boost::asio::transfer_at_least(1),
...
//	std::cout << "Request: " << Req << std::endl;
	...
	boost::asio::async_write(ssocket_, boost::asio::buffer(fReq),
 ...
 * @param len 
 */
void connection::handle_server_write(const boost::system::error_code& err, size_t len) {
// 	std::cout << "handle_server_write. Error: " << err << ", len=" << len << std::endl;
	if(!err) {
		handle_server_read_headers(boost::system::error_code(), 0);
	}else {
		shutdown();
	}
}
 ...
 * @param len 
 */
void connection::handle_server_read_headers(const boost::system::error_code& err, size_t len) {
// 	std::cout << "handle_server_read_headers. Error: " << err << ", len=" << len << std::endl;
	if(!err) {
		...
		if(idx == std::string::npos) { // going to read rest of headers
			async_read(ssocket_, boost::asio::buffer(sbuffer), boost::asio::transfer_at_least(1),
...
// 					  << " isPersist: " << isPersistent << std::endl;
			...
			// sent data
			boost::asio::async_write(bsocket_, boost::asio::buffer(fHeaders),
 ...
 * @param len 
 */
void connection::handle_browser_write(const boost::system::error_code& err, size_t len) {
//   	std::cout << "handle_browser_write. Error: " << err << " " << err.message()
//  			  << ", len=" << len << std::endl;
	if(!err) {
		if(!proxy_closed && (RespLen == -1 || RespReaded < RespLen))
			boost::asio::async_read(ssocket_, boost::asio::buffer(sbuffer,len), boost::asio::transfer_at_least(1),
 ...
 * @param len 
 */
void connection::handle_server_read_body(const boost::system::error_code& err, size_t len) {
